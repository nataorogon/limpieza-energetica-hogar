/**
 * Motas de luz doradas — la traducción de marca del motivo de "energía"
 * (en vez del agua acuarela de la referencia).
 *
 * Van solas: son el único fondo del cuestionario, sin flor ni haz detrás.
 * Se animan siempre, no dependen de --progress ni de data-state; el estilo
 * vive en .motas-luz (globals.css) y viaja con el componente.
 *
...
const MOTES = Array.from({ length: N }, (_, i) => {
  ...
  return {
    ...
    r: 1.4 + ((i * 7) % 3) * 0.7,
    blur: i % 6 === 0,
    // Variables que consume el CSS de .motas-luz en globals.css
    dur: 5.5 + ((i * 11) % 40) / 10, // duración del ciclo de deriva
